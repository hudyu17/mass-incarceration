...
We feel that the primary weakness of our analysis was the limited nature of the intermediate datasets employed in the original paper. Most of the files that fed into the author's plots were already filtered in some way (e.g. scoped down to a single year), resulting in rigid data that made it difficult to manipulate further for new insights. Compounding this issue was the arcane process of obtaining some datasets; despite being publicly available, many sources need to be requested or collated across large amounts of individual files, in many cases making the pre-cleaned versions provided in the replication directory the only viable data source. 

This study could go further with this existing data, or investigate these findings with other datasets. Exploring the prison systems of other countries could provide valuable insight into how incarceration is treated differently, revealing best practices for the benefit of both incarcerated individuals and the general population. 
